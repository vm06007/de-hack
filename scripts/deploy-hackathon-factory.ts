...
async function main() {
    console.log("Deploy & Verify HackathonFactory on Mainnet");
    console.log("═".repeat(50));

    try {
        // Deploy using Hardhat Ignition with echo to handle prompts
        console.log("\nDeploying HackathonFactory contract...");

        // Use yes command to automatically answer all prompts with 'y'
        const deployCommand = `yes | npx hardhat ignition deploy ignition/modules/HackathonFactory.ts --network mainnet --reset`;
        ...
        const addressMatch = deployOutput.match(/HackathonFactoryModule#HackathonFactory - (0x[a-fA-F0-9]{40})/);
        ...
        console.log("\nSummary");
        console.log("═".repeat(50));
        console.log(`Contract: HackathonFactory`);
